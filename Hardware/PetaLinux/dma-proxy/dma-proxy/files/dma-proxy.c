...
static void test(void)
{
	...
	for (i = 0; i < TEST_SIZE; i++)
		...
		//else printk("TX buffer = RX buffer in: %d, with value: %d \n", i, channels[TX_CHANNEL].interface_p->buffer[i]);
...
{
	printk("ioctl entry call.. Inside module rn..\n");
	struct dma_proxy_channel *pchannel_p = (struct dma_proxy_channel *)file->private_data;
	printk("ioctl access data.. Inside module rn..\n");
	/* Perform the DMA transfer on the specified channel blocking til it completes
	 */
	printk("ioctl transfer call.. Inside module rn..\n");
	transfer(pchannel_p);
	printk("ioctl exit call.. Inside module rn..\n");
	return 0;
}
