...
int main(int argc, char *argv[])
{
	...
	{
		pixel = dst.ptr<uchar>(i);  // point to first color in row
	    for(int j = 0; j < dst.cols; j++)
	    {
	    	*pixel++ = rx_proxy_interface_p->buffer[(i*640)+j+18];		//weird shift inside HLS? +16 for filter, +3 for simpledesign...
	    	//dst.at<uchar>(i,j) = rx_proxy_interface_p->buffer[(i*640)+j+3];
	    	//if( (i*640)+j +) printf("data is %d\n", *pdata);
	    	//*pdata = rx_proxy_interface_p->buffer[(i*640)+j+3];
    ...
    cout << "Imginloop  in: " << elapsed_seconds.count() << endl;
    elapsed_seconds = duration_cast<duration<double>>(imgoutST-imgoutS);
    cout << "Imgoutloop in : " << elapsed_seconds.count() << endl;
	return 0;
}
